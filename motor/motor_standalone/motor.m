...
% Calcolo corrente massima approssimativa
% input necessari: parametri motore, U
wmax2 = 1000;
for stp = 1 : 100
    wmax1 = max([ U/2/Ke, min([wmax2, U/Ke ]) ]);
    I = (U - Ke*wmax1) / sqrt( R^2 + (L*wmax1)^2);
    M = (Ke*I - (kL*wmax1-MvK));
    wmax2 = wmax1 + sqrt( R^2 + (L*wmax1)^2)/Ke^2*M;
end
Imax = (U - Ke*5*wmax2/8) / sqrt(R^2+(L*5*wmax2/8)^2);
